...
void AdapterListUpdateCallback::set_adapter(Adapter *p_adapter) {
	m_adapter = p_adapter;
}

Ref<Adapter> AdapterListUpdateCallback::get_adapter() const {
	// Variant path uses reference(), which is safe on an already-referenced
	// object (Ref(T*) would go through init_ref() and release the reference on
	// destruction).
	return Ref<Adapter>(Variant(m_adapter));
}
